...
export default function IGDownloader() {
  ...
  const handleDownloadExtract = async () => {
    ...
    if (!url.includes("instagram.com")) {
      setError("Please provide a valid Instagram link (Reel, Story, or Post).");
      return;
    }
    ...
    try {
      const response = await fetch('/api/ig-downloader', {
        method: 'POST',
        headers: {
      ...
      setMediaData({
        type: data.type || (url.includes("/reel/") || url.includes("/video/") ? 'video' : 'image'),
        url: data.url,
      ...
      setError(error.message || "Failed to fetch media. Make sure the Instagram account is public.");
    } finally {
  ...
  const downloadMedia = async () => {
    ...
    try {
      // Direct binary download from our custom proxy to bypass CORS
      const proxyUrl = `/api/proxy-download?url=${encodeURIComponent(mediaData.url)}`;
      
      const a = document.createElement("a");
      a.href = proxyUrl;
      a.download = `ig-download-${Date.now()}.${mediaData.type === 'video' ? 'mp4' : 'jpg'}`;
          ...
          <div className={`lg:col-span-2 ${cardBaseClass} min-h-[400px] flex flex-col`}>
            ...
            {!mediaData && !isProcessing ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-10 transition-all">
                <div className="w-20 h-20 rounded-full bg-gradient-to-tr from-yellow-400 via-pink-500 to-indigo-500 flex items-center justify-center mb-6">
                  <Instagram className="w-8 h-8 text-white" />
                </div>
                <h3 className="text-xl font-bold mb-4">Paste Instagram Link</h3>
                ...
                <div className="w-full max-w-md flex flex-col gap-4">
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      className="block w-full pl-11 pr-4 py-4 bg-[#1a1a1a] border border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent transition-all"
                  ...
                  <button 
                    ...
                    className={btnPrimaryClass + " !bg-pink-600 hover:!bg-pink-500 !text-white"}
                  >
                    Fetch Content
                  </button>
                </div>
               ...
               <div className="flex-1 flex flex-col items-center justify-center p-10">
                 ...
                 <p className="text-gray-400 text-sm text-center">
                   Fetching media from Instagram. This will just take a moment.
                 </p>
               </div>
            ) : mediaData ? (
              // Result State
              <div className="flex flex-col h-full gap-6">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 p-4 rounded-2xl bg-white/5 border border-white/5 mt-auto">
                  ...
                  <button 
                    ...
                    className={`${btnPrimaryClass} !bg-indigo-600 hover:!bg-indigo-500 !text-white !py-2 !px-6 border-none`}
                  >
